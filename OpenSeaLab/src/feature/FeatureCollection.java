...
public class FeatureCollection implements Serializable {
	 ...
	 * features.iterator(); iterator.hasNext();) { Feature feature =
	 * iterator.next();
	 * 
	 * if (!feature.getGeometry().getType().equals("MultiPolygon")) { continue; }
	 * 
	 * MultiPolygon mp = (MultiPolygon) feature.getGeometry(); if
	 * (mp.getExteriorRings().size() > 0) { List<Polygon> polygons =
	 * mp.getExteriorRings(); for (Polygon p : polygons) { Feature f =
	 * feature.copy(); f.setGeometry(p); deduped.add(f); } toRemove.add(feature); }
